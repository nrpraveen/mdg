   ...
   public 
	...
	private function registerActions() {
		...
		add_action('manage_example_posts_custom_column', function($column, $post_id) {
			if($column == 'example') echo get_post_meta($post_id, 'example_body', true);
			elseif($column == 'attribution') echo get_post_meta($post_id, 'example_attribution', true);
			elseif($column == 'attribution') echo get_post_meta($post_id, 'example_attribution', true);
		}, 10, 2);
		
		add_filter('manage_example_posts_columns', function($columns) {
			unset($columns['title'], $columns['date']);
			$columns['example'] = 'Example Body';
